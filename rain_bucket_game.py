...
GAME_TIMER = 30  # Game duration in seconds
...
# Game Over screen for Individual player
def show_game_over_screen_individual():
    screen.fill(WHITE)
    game_over_text = large_font.render("Game Over", True, BLUE)
    game_over_rect = game_over_text.get_rect(center=(WIDTH // 2, HEIGHT // 2 - 50))
    screen.blit(game_over_text, game_over_rect)
...
def game_loop(game_duration, control_mode):
    ...
    while not game_over:
        ...
        if elapsed_time >= game_duration:
            ...
            if show_game_over_screen_individual():
                # Reset score
                score = 0
                return True  # Return to menu
...
def show_start_screen():
    ...
    start_button = pygame.Rect(WIDTH // 2 - 100, HEIGHT // 2 + 140, 200, 50)
...
while True:
   if show_start_screen() == HUMAN_VS_AI:
        if game_loop_human_vs_ai(GAME_TIMER):
            continue  # Return to menu    

   elif show_start_screen() == HUMAN_CONTROL:
        if game_loop(GAME_TIMER, HUMAN_CONTROL):
            continue
    
   elif show_start_screen() == AI_CONTROL:
        if game_loop(GAME_TIMER, AI_CONTROL):
            continue
